...
namespace Nit
{
    ...
    namespace Internal 
    {
        ...
        static GLenum FBTextureFormatToGL(EFramebufferTextureFormat format)
        {
            switch (format)
            {
            case EFramebufferTextureFormat::RGBA8:       return GL_RGBA8;
            case EFramebufferTextureFormat::RedInteger: return GL_RED_INTEGER;
            }
    ...
    COpenGlFramebuffer::COpenGlFramebuffer(const CFramebufferSpecification& spec)
    ...
    void COpenGlFramebuffer::Invalidate()
    {
        ...
        if (m_ColorAttachmentSpecifications.size())
        {
            ...
            Internal::CreateTextures(multisample, m_ColorAttachments.data(), (uint32_t) m_ColorAttachments.size());
            ...
            for (size_t i = 0; i < m_ColorAttachments.size(); i++)
            {
                ...
                case EFramebufferTextureFormat::RGBA8:
                    Internal::AttachColorTexture(m_ColorAttachments[i], m_Specification.Samples, GL_RGBA8, GL_RGBA, m_Specification.Width, m_Specification.Height, (int) i);
                    break;
                case EFramebufferTextureFormat::RedInteger:
                    Internal::AttachColorTexture(m_ColorAttachments[i], m_Specification.Samples, GL_R32I, GL_RED_INTEGER, m_Specification.Width, m_Specification.Height, (int) i);
                    break;
                }
            }
        }
        ...
        {
            assert((m_ColorAttachments.size() <= 4) && "Invalid size!");
    ...
    int COpenGlFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y)
    {
    ...
    void COpenGlFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
    {
        if ((attachmentIndex < m_ColorAttachments.size()))
        {
            // Invalid index!
        ...
        glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
            Internal::FBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
    }
}